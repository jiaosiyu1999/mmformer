...
from mask2former.data import (
    FewShotDatasetMapper,
    FewShotDatasetMapper_ori,
    build_detection_train_loader,
    build_detection_test_loader,
)
...
def build_train_loader(cfg):
    if cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_sem":
        mapper = FewShotDatasetMapper(cfg, True)
        # print(build_detection_train_loader(cfg, mapper=mapper))
    elif cfg.INPUT.DATASET_MAPPER_NAME == "fewshot_sem_ori":
        mapper = FewShotDatasetMapper_ori(cfg, True)
    else:
        mapper = None
...
def do_train(cfg, model, resume=False, data_loaders = None, evaluators = None):
    ...
    #     start_val = max_iter//2
    # elif cfg.MODEL.BACKBONE.FREEZE_AT == 5:
    #     start_val = 0
...
def setup(args):
    ...
    # 
    print('OUTPUT_DIR', OUTPUT_DIR, 'DATASETS.TRAIN', DATASETS_TRAIN, 'DATASETS.TEST', DATASETS_TEST)
    
     # ['SEED', k]
    cfg.merge_from_list(['OUTPUT_DIR', OUTPUT_DIR, 'DATASETS.TRAIN', DATASETS_TRAIN, 'DATASETS.TEST', DATASETS_TEST]) # ['SEED', k]
    cfg.freeze()
