...
def register_all_coco(root, split, BASE_CLASS_NAMES, NOVEL_CLASS_NAMES):
    root = '/home/siyujiao/data/vspw/VSPW/data'
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        gt_dir = '/home/siyujiao/data/vspw/p2o/data'
        
        # print(all_name, 'x'*500)
        d = 'lists/p2o/' + name + '_list_split' + str(split) + '.pth'
        if name=="train":
            all_name = f"p2o_fewshot_sem_seg_{name}_{str(split)}"
            DatasetCatalog.register(
                all_name,
                lambda d=d: load_fewshot_voc_seg(d, name),
            ...
            shot = 1
            all_name = f"p2o_fewshot_sem_seg_{name}_{str(split)}_1shot"
            DatasetCatalog.register(
                all_name,
                lambda d=d: load_fewshot_val1000(d, name, split = split, shot = shot, dataname = 'p2o'),
                # lambda split=split, name=name: load_fewshot_voc_seg(split, name),
            )
            ...
            shot = 5
            all_name = f"p2o_fewshot_sem_seg_{name}_{str(split)}_5shot"
            DatasetCatalog.register(
                all_name,
                lambda d=d: load_fewshot_val1000(d, name, split = split, shot = shot, dataname = 'p2o'),
                # lambda split=split, name=name: load_fewshot_voc_seg(split, name),
...
_root = '/home/siyujiao/data/vspw/VSPW/data/'

for split in range(4):
    class_list, sub_list, sub_val_list = shot_generate(split, dataname='p2o')
